# AdvancedR2018/Projekt1/KomKruKur/analize_CRAN.R


setwd("h:\\Windows7\\Desktop\\R_level_2\\proj")
...
ulepszanie1 <- microbenchmark(
  ...
  wersjaNowa1 = { 
    w1 <- w2 <- numeric(0)
    w1 <- sapply(df$v_names, function(x) countCharOccurrences(".", x))
    w2 <- sapply(df$v_names, function(x) countCharOccurrences("_", x)) },
  wersjaNowa2 = { 
    w1 <- w2 <- numeric(nrow(df))
    for(i in 1:nrow(df)){
      w1[i] <- countCharOccurrences(".", df$v_names[i])
      w2[i] <- countCharOccurrences("_", df$v_names[i])
    }},
  times=5
)
...
# Najlepsza wersja -> wersjaNowa1
# Lepszy czas ok. 10 razy.
...
# najd³u¿ej dzia³a: c(), $ (na 5 miejscu)
