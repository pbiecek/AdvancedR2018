load("h:/Windows7/Desktop/R_zaawansowany/GIT_PROCESSED_WITH_TA")
#GIT_anal
...
########################
poprawa_1<- microbenchmark(
stara_wersja = {df <- db_git %>%
...
nowa_wersja ={db_git <- data.table(db_git);

  df <- db_git[,.(n_repo=n_distinct(repo_name)), by = user_name]
}  
...
poprawa_2 <- microbenchmark(wersja_1 = {w1 <- w2 <- numeric(0);
for(i in 1:nrow(df)){
...
w1 <- sapply(df$v_names, function(s) countCharOccurrences(".", s) );
w2 <- sapply(df$v_names, function(s) countCharOccurrences("_", s) );

},
wersja_4 ={
  x <- df$v_names;
  w1 <- sapply(x, function(s) countCharOccurrences(".", s) );
  w2 <- sapply(x, function(s) countCharOccurrences("_", s) );
